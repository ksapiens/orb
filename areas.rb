...
class Writer < Pad #Area 
	...
	def initialize args 
		...
		unless oneline?
		#if @height > 2
			@header = Window.new 1, @width, @y, @x
			@footer = Window.new 1, @width, @height+@y-1, @x
			@height -= 2
			@y += 1
		end
		super 1000, @width
		...
		#draw "test"
		update
		#LOG.debug "cnt %s" % [@content]
		#LOG.debug "pad %s %s %s %s %s %s" % [top, left, bottom,right,height,width]
